...
useEffect(() => {
  ...
  // Component unmount olunca temizleme
  return () => {
    document.body.style.overflow = "auto";
  };
}, [modal]);
  ...
  return (
    <div className=" w-full min-h-screen  flex flex-col items-center  space-y-5 py-24  ">
      ...
      ${productStyleColumn ? 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 max-md:w-[75%]' : 'lg:w-[80%] md:grid-cols-2'} p-5`}>
        ...
        {
          <div
          className={`fixed inset-0 flex items-center justify-center bg-black/50 z-50  ${modal ? "opacity-100 scale-100  " : "opacity-0 scale-50 max-h-0"}`}
          onClick={() => setModal(false)} // Arka plana tıklayınca kapat
        >
          <div
            className={`bg-white dark:bg-gray-700 p-5 rounded-lg flex items-center justify-center flex-col space-y-3 shadow-lg w-[90%] max-w-md relative 
               transition-all duration-700 ease-in-out
            ${modal ? "scale-100 opacity-100" : "scale-0 max-h-0 opacity-0"}`}
            onClick={(e) => e.stopPropagation()} // Modal içeriğine tıklayınca kapanmasını engelle
          >
            <button 
              onClick={() => setModal(false)} 
              className="absolute top-2 right-2 p-1 border border-gray-500 rounded-lg text-xl"
            >
              <IoCloseSharp />
            </button>
            <h2 className="text-xl font-bold">{selectedProduct?.title}</h2>
            <img src={selectedProduct?.image} alt={selectedProduct?.title} className="w-full h-56 object-cover rounded-lg" />
            <p
                className="text-lg"
                dangerouslySetInnerHTML={{ __html: selectedProduct?.content }} />
            <h3 className="text-xl w-full bg-gray-900 p-2 rounded-lg flex text-center justify-center font-semibold text-green-500 mt-2">{selectedProduct?.price} TL</h3>
          </div>
        </div>
      }
      ...
      {userProducts?.map((product, index) => (
          <li
            key={index}
            onClick={(e) => {
              const rect = e.currentTarget.getBoundingClientRect(); // Tıklanan elemanın konumu
              setModalPosition({ top: rect.top + window.scrollY, left: rect.left }); 
              setSelectedProduct(product);
              setModal(true);
            }}
            className={`cursor-pointer  p-2 dark:shadow-none shadow-md shadow-gray-400  transition-all duration-300 
