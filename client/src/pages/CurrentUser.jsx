...
<h1 className='p-3 rounded-xl bg-sky-600 text-white text-2xl'>{username}</h1>
    ...
    <div className='overflow-auto  w-full grid grid-cols-1 max-[429px]:px-14 min-[429px]:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 justify-items-center lg:w-[1024px]  gap-5 p-5'>
    {
        userCategory.map((ctg,index)=>(
            <Link to={`products?tab=${ctg.name}`}
             key={index*2} 
            className='w-full min-h-48 rounded-lg dark:shadow-none shadow-md shadow-gray-400 
            ...
            <div 
                        ...
                        bg-no-repeat rounded-lg  flex flex-col items-center justify-center text-center'
                        ...
                        <h1 key={index}
                         className=
                        'p-2  w-full bg-sky-600    rounded-lg text-white font-extrabold text-xl flex items-center justify-center'>
